...
/**
 * DiagramSchema will hold the palette colors, line widths and similar
 * information for the DiagramView.
 * 
...
public class DiagramSchema implements Comparable {
    ...
    static {
        try {
            ...
            if (children.length != 0) {
                ...
                final DiagramSchema userSchema = getDefaultSchema();
    ...
    private static DiagramSchema getWhiteNodeSchema() {
        final DiagramSchema schema = new DiagramSchema();
        ...
        schema.arrowStyles = new ArrowStyle[] { new ArrowStyle(Color.RED),
                new ArrowStyle(Color.BLUE), new ArrowStyle(Color.GREEN),
                new ArrowStyle(Color.CYAN), new ArrowStyle(Color.GRAY),
                new ArrowStyle(Color.MAGENTA), new ArrowStyle(Color.ORANGE),
                new ArrowStyle(Color.PINK), new ArrowStyle(Color.BLACK),
    ...
    /**
     * @todo the default values are duplicated in the
     *       restore(ExtendedPreferences) method -- remove
     */
    ...
    private static DiagramSchema readFromPreferences(final Preferences prefs) {
        final ExtendedPreferences extPrefs = new ExtendedPreferences(prefs);
        final DiagramSchema schema = new DiagramSchema();
    ...
    /**
     * Returns the type of information that should be used to create the
     * diagram.
     */
    ...
    public void setDefaultLineWidth(final float lineWidth) {
    ...
    public void setSelectionLineWidth(final float lineWidth) {
    ...
    public void setNodeStrokeWidth(final float nodeStrokeWidth) {
     ...
     */
    public void setCircleColor(final Color circleColor) {
    ...
    public void setGradientType(
            final ConceptInterpreter.IntervalType gradientType) {
     ...
     */
    public Color fadeOut(final Color original) {
        return new Color(
                (int) (original.getRed() * (1 - fadeOut) + 255 * fadeOut),
    ...
    public void setNodeSizeScalingType(
            final ConceptInterpreter.IntervalType nodeSizeScalingType) {
        this.nodeSizeScalingType = nodeSizeScalingType;
    }

    public Color getNotRealisedNodeColor(final Color nodeColor) {
    ...
    private void writeToPreferences(final Preferences prefs) {
        ...
        extPrefs
                .putColor("nestedDiagramNodeColor", this.nestedDiagramNodeColor);
        extPrefs.putColor("notRealisedDiagramNodeColor",
                this.notRealisedDiagramNodeColor);
        ...
        extPrefs.putFloat("notRealizedNodeSizeReductionFactor",
                this.notRealizedNodeSizeReductionFactor);
        ...
        for (int i = 0; i < this.arrowStyles.length; i++) {
            final ArrowStyle style = this.arrowStyles[i];
            final Preferences stylePrefs = extPrefs.node("arrowStyle-" + i);
    ...
    public static void storeAll() {
        for (final DiagramSchema schema : schemas) {
            schema.store();
        }
    }
    ...
    public void restore(final ExtendedPreferences extPrefs) {
        this.background = extPrefs.getColor("backgroundColor", null);
        this.topColor = extPrefs.getColor("topColor", new Color(0, 0, 150));
        this.bottomColor = extPrefs.getColor("bottomColor", new Color(255, 255,
                150));
        this.gradient = this.getDefaultGradient();
        this.foreground = extPrefs.getColor("foregroundColor", new Color(0, 0,
                0));
        this.nestedDiagramNodeColor = extPrefs.getColor(
                "nestedDiagramNodeColor", new Color(255, 255, 255));
        this.notRealisedDiagramNodeColor = extPrefs.getColor(
                "notRealisedDiagramNodeColor", null);
        this.circleColor = extPrefs.getColor("circleColor", new Color(0, 0, 0));
        this.lineColor = extPrefs.getColor("lineColor", new Color(0, 0, 0));
        this.circleSelectionColor = extPrefs.getColor("circleSelectionColor",
                new Color(255, 0, 0));
        this.circleIdealColor = extPrefs.getColor("circleIdealColor",
                new Color(0, 0, 0));
        this.circleFilterColor = extPrefs.getColor("circleFilterColor",
                new Color(0, 0, 0));
        this.fadeOut = extPrefs.getFloat("fadeOutValue", 0.7F);
        this.margin = extPrefs.getInt("margin", 20);
        this.notRealizedNodeSizeReductionFactor = extPrefs.getFloat(
                "notRealizedNodeSizeReductionFactor", 3);
        ...
        if (propVal.equals("extent")) {
            ...
            System.err
                    .println("Caught unknown gradient type for DiagramSchema: "
                            + propVal);
        ...
        final String labelFontName = extPrefs.get("labelFontName", "SansSerif");
        final int labelFontSize = extPrefs.getInt("labelFontSize", 10);
        ...
        if (propVal.equals("contingent")) {
            ...
            System.err
                    .println("Caught unknown node size scaling value for DiagramSchema: "
                            + propVal);
            System.err.println("-- using default");
        }
        final List<ArrowStyle> arrowStylesList = new ArrayList<ArrowStyle>();
        int i = 0;
        try {
            while (extPrefs.nodeExists("arrowStyle-" + i)) {
                arrowStylesList.add(new ArrowStyle(extPrefs.node("arrowStyle-"
                        + i)));
                i++;
            }
        } catch (final BackingStoreException e) {
            // make sure to revert to defaults -- otherwise we might not have
            // enough
            arrowStylesList.clear();
        }
        if (arrowStylesList.size() != 0) {
            this.arrowStyles = arrowStylesList
                    .toArray(new ArrowStyle[arrowStylesList.size()]);
        } else {
            this.arrowStyles = new ArrowStyle[] { new ArrowStyle(Color.RED),
                    new ArrowStyle(Color.BLUE), new ArrowStyle(Color.GREEN),
                    new ArrowStyle(Color.CYAN), new ArrowStyle(Color.GRAY),
                    new ArrowStyle(Color.MAGENTA),
                    new ArrowStyle(Color.ORANGE), new ArrowStyle(Color.PINK),
                    new ArrowStyle(Color.BLACK), new ArrowStyle(Color.YELLOW) };
        }
    }
}
