...
public class ExportDiagramAction extends KeyboardMappedAction {
     ...
     */
    public ExportDiagramAction(final Frame frame,
            final DiagramExportSettings diagExpSettings,
            final DiagramView diagramView) {
        super(frame, "Export Diagram...");
        this.diagramExportSettings = diagExpSettings;
        this.diagramView = diagramView;
        this.exportSettingsPanel = new DiagramExportSettingsPanel(
                this.diagramExportSettings);
    }

    public ExportDiagramAction(final Frame frame,
            final DiagramExportSettings diagExpSettings,
            final DiagramView diagramView, final int mnemonic,
    ...
    public void exportImage() {
        // important: getParentFile() -- otherwise it won't work if the exported
        // image was deleted
        final CustomJFileChooser saveDialog;
        final File lastImageExportFile = this.diagramExportSettings
                .getLastImageExportFile();
        if (lastImageExportFile != null) {
            saveDialog = new CustomJFileChooser(lastImageExportFile
                    .getParentFile());
        } else {
        ...
        while (formatIterator.hasNext()) {
            final GraphicFormat graphicFormat = (GraphicFormat) formatIterator
                    .next();
            final ExtensionFileFilter fileFilter = new ExtensionFileFilter(
                    graphicFormat.getExtensions(), graphicFormat.getName());
        ...
        do {
            ...
            if (rv == JFileChooser.APPROVE_OPTION) {
                ...
                final GraphicFormat gFormat = GraphicFormatRegistry
                        .getTypeByName(extFileFilter.getFileTypeName());
                if (gFormat != null) {
        ...
        try {
            ...
            if (diagramHistory.getNumberOfCurrentDiagrams() != 0) {
                ...
                for (int i = 0; i < numCurDiag; i++) {
                    title += diagramHistory.getElementAt(i + firstCurrentPos)
                            .toString();
                    if (i < numCurDiag - 1) {
                        title += " / ";
                    }
                ...
                description = diagramController.getDiagramHistory()
            ...
            final ImageWriter writer = this.diagramExportSettings
                    .getGraphicFormat().getWriter();
            writer.exportGraphic(this.diagramView, this.diagramExportSettings,
            ...
            if (this.diagramExportSettings.getSaveCommentsToFile() == true) {
                try {
                    ...
                    out
                            .println("The diagram(s) you have viewed for the resulting image: "
                                    + lineSeparator
                                    + selectedFile.getAbsolutePath());
                    final DateFormat dateFormatter = DateFormat
                            .getDateTimeInstance();
                    out.println("as at "
                            + dateFormatter.format(new Date(System
                                    .currentTimeMillis())) + " is(are): ");
                    out.println();
                    out.println(description);
                    out.close();
                } catch (final IOException e) {
                    ErrorDialog.showError(this.frame, e,
                            "Exporting text file error");
                }
            }
            if (this.diagramExportSettings.getSaveCommentToClipboard() == true) {
                ...
                final StringSelection comments = new StringSelection(header
                        + "\n" + description);
                final Clipboard systemClipboard = java.awt.Toolkit
            ...
            ErrorDialog.showError(this.frame, e, "Out of memory",
    ...
    private class CustomJFileChooser extends JFileChooser {
        ...
        public void approveSelection() {
            ...
            if (selectedFile.exists()) {
                String warningMessage = "The image file '"
                        + selectedFile.getName()
                        + "' already exists.\nDo you want to overwrite the existing file?";
                if (ExportDiagramAction.this.diagramExportSettings
                        .getSaveCommentsToFile() == true) {
                    final File textFile = new File(selectedFile
                            .getAbsoluteFile()
                            + ".txt");
                    if (textFile.exists()) {
                        warningMessage = "The files '"
                                + selectedFile.getName()
                                + "' and '"
                                + textFile.getName()
            ...
            if (!selectedFile.exists()
                    && ExportDiagramAction.this.diagramExportSettings
                            .getSaveCommentsToFile() == true) {
                final File textFile = new File(selectedFile.getAbsoluteFile()
                        + ".txt");
                if (textFile.exists()) {
