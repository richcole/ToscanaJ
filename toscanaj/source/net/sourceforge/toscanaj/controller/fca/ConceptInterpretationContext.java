...
/**
 * @todo remove the objectDisplayMode out of this class, it is not really part
 *       of the interpretetation context, but an aspect of a certain view.
 *       Having it in here caused troubles with requerying since the interpreter
 *       assumed a context change whenever the view was changed and thus queried
 *       the same information again. This is by now avoided by not using the
 *       display mode in equals()/hashCode(), but this is of course only half a
 *       solution.
 * 
 * @param <O>
...
public class ConceptInterpretationContext<O, A> implements ChangeObserver {
    ...
    private final DiagramHistory diagramHistory;

    // / @todo allow passing null brokers
    private final EventBroker eventBroker;

    private final List<Concept<O, A>> nestingConcepts = new ArrayList<Concept<O, A>>();
    private final List<ConceptInterpretationContext<O, A>> nestingContexts = new ArrayList<ConceptInterpretationContext<O, A>>();

    // / @todo use something else than diagramHistory as first parameter -- not
    // useful in anything but Toscana, even not
    // / in the editors
    public ConceptInterpretationContext(final DiagramHistory diagramHistory,
    ...
    public ConceptInterpretationContext<O, A> createNestedContext(
            final Concept<O, A> nestingConcept) {
        final ConceptInterpretationContext<O, A> retVal = new ConceptInterpretationContext<O, A>(
                this.diagramHistory, this.eventBroker);
    ...
    public void setObjectDisplayMode(final boolean isContingent) {
        this.objectDisplayMode = isContingent;
        // this change is not considered to be relevant anymore, consistently
        // with equals() and hashCode()
        // this.eventBroker.processEvent(new
        // ConceptInterpretationContextChangedEvent(this));
    }
    ...
    public void setFilterMode(final boolean isContingent) {
        this.filterMode = isContingent;
        this.eventBroker
                .processEvent(new ConceptInterpretationContextChangedEvent(this));
    }
    ...
    public Concept<O, A> getOutermostTopConcept(final Concept<O, A> concept) {
        if (this.nestingConcepts.size() == 0) {
            return concept.getTopConcept();
        } else {
            final Concept<O, A> outermostConcept = this.nestingConcepts.get(0);
    ...
    public boolean equals(final Object other) {
        ...
        final ConceptInterpretationContext<?, ?> otherContext = (ConceptInterpretationContext<?, ?>) other;
