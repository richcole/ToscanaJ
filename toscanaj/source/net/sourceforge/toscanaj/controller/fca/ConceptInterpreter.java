 ...
 * 
 * Instead of using concepts directly, a concept interpreter can be used to
 * replace them with updated information. A typical use-case is the classic
 * Toscana-system: the objects in the predefined diagrams are SQL queries, the
 * actual objects of interest are the ones found in the database. The concept
 * interpreter's role is to map one to the other.
 * 
 * Another role of the concept interpreter is to consider the context in which a
 * concept is used. The concept stored is usually the one of a full data set,
 * filtering and nesting can reduce the available data, thus changing the
 * concept, potentially even turning it into one that wouldn't normally be there
 * since it has the same extent/intent as others (a so-called
 * "unrealized concept"). The concept interpreter does this mapping, too.
 * 
 * @todo check if it would be better to separate the two tasks of
 *       contextualization and realization.
 * 
 * @param <Oc>
 *            The type of the objects in the concrete scale.
 * @param <A>
 *            The type of the attributes used in both scales (always the same).
 * @param <Or>
 *            The type of the objects in the realized scale.
 */
public interface ConceptInterpreter<Oc, A, Or> {
    ...
    Iterator<A> getAttributeContingentIterator(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);

    /** independent of displayMode and filterMode */
    Iterator<A> getIntentIterator(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);

    /** is dependent on displayMode and filterMode */
    int getObjectCount(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);

    /** is dependent on displayMode and filterMode */
    int getAttributeCount(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);

    /** these are independent of displayMode and dependent on filterMode */
    int getObjectContingentSize(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);

    /** these are independent of displayMode and dependent on filterMode */
    int getExtentSize(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);

    NormedIntervalSource<Oc, A> getIntervalSource(IntervalType type);

    boolean isRealized(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);
     ...
     * @param concept
     *            The concept to execute the query upon.
     * @param context
    ...
    boolean isVisible(Concept<Oc, A> concept,
            ConceptInterpretationContext<Oc, A> context);
}
