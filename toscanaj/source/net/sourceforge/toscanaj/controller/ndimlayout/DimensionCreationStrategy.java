 ...
 * 
 * A conceptual dimension is a set of attributes that belong together, such as
 * all single-valued attributes from an ordinal scale. This information can then
 * be used to layout the diagram in a way that an attribute-additive diagram
 * uses the same vector for each attribute of a dimension.
 * 
 * Note that this class does not consider the objects as relevant, the matching
 * type parameter is completely free.
 * 
 * @param <A>
 *            The type of attributes to consider, which allows specific
 *            strategies to operate only on certain attributes.
 */
